...
class ParserTestCase(unittest.TestCase):
    ...
    def test_parses_rss(self):
        parsed_content = self.parser.parse(SAMPLE_RSS)
        assert_that(parsed_content,
                    equal_to(ParsedContent(timestamp=(2022, 7, 27, 11, 45, 17, 0),
                        items=[AvailableItem(alert='Stock Alert (DE): RPi CM4 - 1GB RAM, 8GB MMC, With Wifi is In Stock at Reichelt',
                                    vendor='reichelt',
                                    location='DE',
                                    product='CM4',
                                    item_timestamp=(2022, 7, 27, 11, 45, 17, 0))])))
